...
const AdminTeacher = () => {
  ...
  useEffect(() => {
    axios.get('/api/get/teacher').then((res) => {
      setList(res?.data?.teacher);
    })
  }, [])
  const show = (id) => {
    console.log(id, '--id');
    axios.post('/api/admin/teacher', { id: id }).then((res) => {
  ...
  const search = (values) => {
    axios.post('/api/admin/searchteacher', { id: values }).then((res) => {
  ...
  const finish = (values) => {
    // console.log(values);
    axios.post('/api/set/teacher', { values: values }).then((res) => {
      ...
      <div className="container">
        <div className='top'>
          ...
          <Breadcrumb>
            ...
            <Breadcrumb.Item>
              <Link to="/admin/teacher" className="link">教师列表</Link>
            </Breadcrumb.Item>
          </Breadcrumb>
        </div>
        <Divider className="divider" />
        <div className="btn">
          <Button size="small" type="primary" onClick={set}>教师信息录入</Button>
        </div>
        <Divider className="divider" />
        <Input.Search
          // loading
        ...
        />
        <Divider className="divider" />
        <Student list={list} type="teacher" show={show} />
        {
          isVisible && <Modal
            maskClosable={false}
            title="信息"
